...
function tipo()
{
        ...
        datos.codg_insti_new.value="$codg_insti";
        }
        datos.submit();
}
function condicion()
...
if ($pasada != 1)
  {
     $consulta_datos_lab=mysql_query("SELECT codg_per, codg_insti, codg_dir, codg_uni, codg_coor, codg_cargo, codg_cat_cargo,
                 codg_jer_cargo, codg_jer_plantel, codg_cond_lab, fec_ingp_lab, turno_lab, dedic_lab, horas_doc_lab, horas_adm_lab,
                 fec_ret, obser_ret, sueldo_integral, bono_bolivariano, bono_simoncito, cesta_ticket
                 FROM bdc_datos_lab
                 WHERE codg_per=$codg_per AND codg_mov=$codg_mov");

                $datos = mysql_fetch_array($consulta_datos_lab);
                $codg_per = $datos["codg_per"];
                $codg_insti = $datos["codg_insti"];
                $codg_dir = $datos["codg_dir"];
                $codg_uni = $datos["codg_uni"];
                $codg_coor = $datos["codg_coor"];
                $codg_cargo = $datos["codg_cargo"];
                $codg_cat_cargo = $datos["codg_cat_cargo"];
                $codg_jer_cargo = $datos["codg_jer_cargo"];
                $codg_jer_plantel = $datos["codg_jer_plantel"];
                $codg_cond_lab = $datos["codg_cond_lab"];
                $fec_ingp_lab = $datos["fec_ingp_lab"];
                $fec_ingp_lab = substr($fec_ingp_lab,8,2)."-".substr($fec_ingp_lab,5,2)."-".substr($fec_ingp_lab,0,4);
                $turno_lab = $datos["turno_lab"];
                $dedic_lab = $datos["dedic_lab"];
                $horas_doc_lab = $datos["horas_doc_lab"];
                $horas_adm_lab = $datos["horas_adm_lab"];
                $hora1 = substr($horas_doc_lab,0,2);
                $decimal1 = substr($horas_doc_lab,2,2);
                $hora2 = substr($horas_adm_lab,0,2);
                $decimal2 = substr($horas_adm_lab,2,2);
                $fec_ret = $datos["fec_ret"];
                $fec_ret = substr($fec_ret,8,2)."-".substr($fec_ret,5,2)."-".substr($fec_ret,0,4);
				if ($fec_ret == '00-00-0000') { $fec_ret = ""; }
                $obser_ret = $datos["obser_ret"];
		/// DATOS NOMINA ////			
				$sueldo_integral = $datos["sueldo_integral"];
				$bono_bolivariano = $datos["bono_bolivariano"];
				$bono_simoncito = $datos["bono_simoncito"];
				$cesta_ticket = $datos["cesta_ticket"];
...
if ($action == "ins")
{
    ...
    //    {
        ...
        $qry=("UPDATE bdc_datos_lab SET codg_insti=$codg_insti_new,
                         codg_dir=$codg_dir, codg_uni=$codg_uni, codg_coor=$codg_coor, codg_cargo=$codg_cargo,
                         codg_cat_cargo=$codg_cat_cargo, codg_jer_cargo=$codg_jer_cargo,
                         codg_jer_plantel='$codg_jer_plantel', codg_cond_lab=$codg_cond_lab,
                         fec_ingp_lab='$fec_ingp_lab', turno_lab='$turno_lab', dedic_lab='$dedic_lab',
                         horas_doc_lab=$horas_doc_lab, horas_adm_lab=$horas_adm_lab, fec_ret='$fec_ret', obser_ret=$obser_ret, sueldo_integral=$sueldo_integral, bono_bolivariano=$bono_bolivariano, bono_simoncito=$bono_simoncito, cesta_ticket=$cesta_ticket WHERE codg_per=$codg_per AND codg_insti=$codg_insti");
...
<FORM METHOD="POST" NAME="datos" action="">
        ...
        <TABLE BORDER="0" ALIGN="CENTER">
               ...
               ?>
                ...
                <TD COLSPAN="3"><SELECT class="campo" NAME="codg_insti_new" title="Seleccione de la lista la institución">
                ...
                if (($codg_insti != 0) && ($codg_insti != ""))
                    {
                        ...
                        echo '<TD><INPUT TYPE="TEXT" NAME="fec_ingp_lab" class="campo" VALUE="'.$fec_ingp_lab.'" MAXLENGTH="10" SIZE="10" id="fec_ingp_lab" onClick="popUpCalendar(this, datos.fec_ingp_lab, '.$formato_fecha.');" READONLY> 
            <IMG onClick="popUpCalendar(this, datos.fec_ingp_lab, '.$formato_fecha.');" SRC="../images/cal.gif" ALT="Haz Click para Buscar la Fecha" WIDTH="16" HEUGHT="16" BORDER="0"></TD>';
                                        echo '</TR>';
                         ...
                         if (mysql_num_rows($categorias) != 0)
                            ...
                            echo '</SELECT>
                                        ...
                                        echo  '<TR>
                                              ...
                                              <TD COLSPAN="3"><SELECT class="campo" NAME="codg_jer_cargo" title="Seleccione de la lista jerarquia">
                                              <OPTION value="0">Seleccione...</OPTION>';
                   ...
                   if ($codg_cond_lab > 1)
                     {
                        ...
                        echo '<TD><INPUT TYPE="TEXT" NAME="fec_ret" class="campo" VALUE="'.$fec_ret.'" MAXLENGTH="10" SIZE="10" id="fec_ret" onClick="popUpCalendar(this, datos.fec_ret, '.$formato_fecha.');" READONLY> 
                  ...
                  if ($codg_tip_trab == "D")
                    {
                           echo '<TD><P ALIGN="RIGHT" class="mini">Horas Docentes:</P></TD>
                                        <TD><P class="campo"><INPUT class="campo" TYPE="TEXT" NAME="hora1" MAXLENGTH="2" SIZE="1" VALUE="'.$hora1.'">.<INPUT class="campo" TYPE="TEXT" NAME="decimal1" MAXLENGTH="2" SIZE="1" VALUE="'.$decimal1.'" title="Introduzca las horas docentes"></P></TD>';
                        }
                ?>
                </TR>
<?		/// DATOS NOMINA //// ?>
...
<SCRIPT LANGUAGE="JavaScript" TYPE="text/javascript">
  ...
  frmvalidator.addValidation("codg_insti_new","dontselect=0","Seleccione una Institución");
 ...
 <?
  if (($sueldo_integral != "") or ($bono_bolivariano != "" ) or ($bono_simoncito != "" ) or ($cesta_ticket != "" ))
  {
 // echo 'frmvalidator.addValidation("sueldo_integral","num");';
  
 // echo 'frmvalidator.addValidation("bono_bolivariano","num");';
  
 // echo 'frmvalidator.addValidation("bono_simoncito","num");';
  
//  echo 'frmvalidator.addValidation("cesta_ticket","num");';
 }
  ?>
