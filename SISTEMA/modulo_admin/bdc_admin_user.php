...
<BR><BR>
<H2>Edici&oacute;n de Usuarios</H1>

<?
if ($action == "elm")
{
     mysql_query("DELETE FROM bdc_usuarios WHERE codg_usr=$user");
...
if ($action == "bus")
{
     ...
     $consulta_usuarios = mysql_query("SELECT codg_usr, apel_usr, nomb_usr, login_usr FROM bdc_usuarios WHERE codg_grp=$codg_grp ORDER BY 3,2");
          ...
          echo '<TD><P class="campo" ALIGN="CENTER">';
                  echo '<A HREF="bdc_edit_user.php?user='.$resultado_usuarios["codg_usr"]; echo '">'.$resultado_usuarios["login_usr"]; echo '</A></P></TD>';
          echo '<TD><P class="mini" ALIGN="CENTER">';
                  echo '<A HREF="bdc_cambio_cont_user.php?user='.$resultado_usuarios["codg_usr"]; echo '">&nbsp;&nbsp;&nbsp;Contrase&ntilde;a</A></P></TD>';
          echo '<TD><P class="mini" ALIGN="CENTER">';
                  echo '<A HREF="bdc_admin_user.php?user='.$resultado_usuarios["codg_usr"]; echo '&action=elm">&nbsp;&nbsp;&nbsp;Eliminar</P></A></TD>';
          echo '<TR>';
          }

          echo '<TR></TR>';
          echo '<TR>';
          echo '<TD COLSPAN="6"><P class="mini" ALIGN="CENTER">Se han encontrado '.mysql_num_rows($consulta_usuarios); echo ' registro(s)</P></TD>';
          echo '</TR>';
          echo ' </TABLE>';
...
?>
        ...
        <BR>
        <FORM METHOD="POST" NAME="grupos" ACTION="bdc_admin_user.php">
        <TABLE BORDER="0" CELLPADDING="0" ALIGN="CENTER">
